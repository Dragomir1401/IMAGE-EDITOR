...
void crop_image(picture *photo)
{
    ...
    if (photo->type == 2 || photo->type == 1 || photo->type == 4 || photo->type == 5)
    {
        ...
        // printf("%d\t%d\n", copy.size.height, copy.size.width);
        ...
        for (uint_fast32_t i = photo->start.height; i < photo->stop.height; i++)
            for (uint_fast32_t j = photo->start.width; j < photo->stop.width; j++)
                copy.bw[i - photo->start.height][j - photo->start.width] = photo->bw[i][j];
        //printf("%d %d\n", photo->stop.height, photo->stop.width);
        free_image(photo);
    ...
    else if (photo->type == 3 || photo->type == 6)
    {
        ...
        for (uint_fast32_t i = photo->start.height; i < photo->stop.height; i++)
            for (uint_fast32_t j = photo->start.width; j < photo->stop.width; j++)
            {
                copy.red[i - photo->start.height][j - photo->start.width] = photo->red[i][j];
                copy.green[i - photo->start.height][j - photo->start.width] = photo->green[i][j];
                copy.blue[i - photo->start.height][j - photo->start.width] = photo->blue[i][j];
            }
        //printf("%d %d\n", photo->stop.height, photo->stop.width);
        free_image(photo);
        photo->size = copy.size;
